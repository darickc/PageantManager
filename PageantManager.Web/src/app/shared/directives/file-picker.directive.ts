...
import { ReadMode } from '../models/read-mode';
...
export class FilePickerDirective implements OnInit {
  ...
  @Input('appFilePicker') readMode: ReadMode;
  ...
  private readFile(file: File): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (loaded: ProgressEvent) => {
        ...
        const pickedFile = new PickedFile(file.lastModifiedDate, file.name, file.size, file.type, this.readMode, fileReader.result);
